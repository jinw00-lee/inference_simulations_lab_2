...
```{r}
tea_data_highn <- make_tea_data(n_total = 48, delta = .75)

tea_data_highn_cond <- tea_data_highn %>%
  group_by(condition) %>%
...
samps_highn <- tibble(sim = 1:1000) |>
  mutate(data = map(sim, \(i) make_tea_data(n_total = 48, delta = 1.5))) |>
  unnest(cols = data)
```
...
tea_data_summary <- samps |>
  group_by(sim, condition) |> # compressing samples in terms of simulation index and conditions
  summarise(mean_rating = mean(rating)) |> # calculating the mean of ratings for each sim index * condition
  group_by(sim) |> # compressing samples in terms of simulation index
  summarise(delta = mean_rating[condition == "milk first"] - # calculating ATE of the tea/milk order for each sim index. 
...
```{r}
hist(tea_data_summary$delta, breaks = 10)
```
...
> When the sample size becomes larger, the histogram becomes more similar to normal distribution.

```{r}
hist(tea_data_highn_summary$delta, breaks = 10)
```
...
samps_highn_perm_sim <- tibble(sim = 1:1000) |>
  mutate(data = map(sim, \(i) make_tea_data(n_total = 48, delta = 0.0))) |>
  unnest(cols = data)

tea_data_highn_perm_sim_summary <- samps_highn_perm_sim |>
  group_by(sim, condition) |> 
  summarise(mean_rating = mean(rating)) |> 
  group_by(sim) |> 
  summarise(delta = mean_rating[condition == "milk first"] -
...
```{r}
hist(tea_data_highn_perm_sim_summary$delta, breaks = 50)
```
...
> "sim" indicates the index of the experiment (i.e., from 1 to 1,000).
...
> This histogram describes normal distribution of null effects (i.e., ATE in the order of tea/milk when the condition labels were randomly shuffled). For this reason, the mean value of the null distribution is positioned close to zero.

```{R}
hist(tea_data_highn_shuffled_summary$delta, breaks = 50) #what does this histogram look like?
```
...
```{r}
ggplot(data = tea_data_highn_shuffled %>% filter(sim == 1000),  # can change the actual simulation number here
...
```{r}
ggplot(data=tea_data_highn_shuffled_summary, aes(x=delta)) +
  geom_histogram(alpha=.4, bins=50, color='blue', fill='blue') +
  geom_histogram(alpha=.8, bins=50, data=tea_data_highn_summary)
```
...
> Each dot represents participant's rating score and their range indicate the \[min, max\] value of observed ratings.

**To do:** how does this change when you use the low-n experiment?

> The 95% CI gets longer when the sample size becomes smaller.

**To do:** Find the shortcut ggplot function so you don't have to calculate these by hand ever again :)

> 'geom_errorbar()' might be helpful in this case.
...
for (this_sim in 1:1000) {
  this_experiment = samps_highn_perm_sim %>%
    filter(sim==this_sim) 
...
hist(all_results$pvalue, breaks = 10)
```

What is the distribution of p-values when the null is true?

> uniform distribution? (totally random?)
...
sum(all_results$pvalue < 0.05)/length(all_results$pvalue)
```
...
```{r}
hist(all_results_high_n$pvalue, breaks = 10)
```
